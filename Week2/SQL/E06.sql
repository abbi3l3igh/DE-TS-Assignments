...
SELECT DISTINCT DATEPART(year, created_ts) created_year, COUNT(1) user_count
FROM users
GROUP BY DATEPART(year, created_ts);

-- Exercise 2
SELECT user_id, user_dob, user_email_id, DATENAME(WEEKDAY, user_dob) user_day_of_birth
FROM users
WHERE DATEPART(MONTH, user_dob) = 5
ORDER BY DATEPART(DAY, user_dob);

-- Exercise 3
SELECT user_id, UPPER(CONCAT(user_first_name, ' ', user_last_name)) [user_name], user_email_id, created_ts, DATEPART(YEAR, created_ts) created_year
FROM users
WHERE DATEPART(YEAR, created_ts) = 2019
ORDER BY user_name;

-- Exercise 4
SELECT DISTINCT user_gender = CASE user_gender
					WHEN 'M' THEN 'Male'
					WHEN 'F' THEN 'Female'
...
-- Exercise 5
SELECT user_id, user_unique_id, user_unique_id_last4 = CASE
									WHEN user_unique_id IS NULL THEN 'Not Specified'
									WHEN (LEN(user_unique_id) < 9) THEN 'Invalide Unique Id'
									ELSE RIGHT(REPLACE(user_unique_id, '-', ''), 4)
									END
FROM users;

-- Exersice 6
-- TODO: COME GROUP BY CONTRY CODE
SELECT CAST((Select TOP 1 * 
			FROM string_split(REPLACE(user_phone_no, '+', ''), '(')) AS INT) [country_code], 
		COUNT(1) [user_count]
FROM users u
WHERE (Select TOP 1 * FROM string_split(REPLACE(user_phone_no, '+', ''), '(')) IS NOT NUll
...
FROM order_items oi
WHERE NOT CAST((oi.order_item_quantity * oi.order_item_product_price) AS DECIMAL(18,2)) = oi.order_item_subtotal;

--Exercise 8
SELECT IIF(DATENAME(WEEKDAY, order_date) LIKE 'Sunday' OR DATENAME(WEEKDAY, order_date) = 'Saturday', 'Weekend Day', 'Week Day')[day_type],
 COUNT(*)[order_count]
FROM orders
WHERE FORMAT(order_date, 'yyyy-MM') = '2014-01'
GROUP BY IIF(DATENAME(WEEKDAY, order_date) LIKE 'Sunday' OR DATENAME(WEEKDAY, order_date) = 'Saturday', 'Weekend Day', 'Week Day')
